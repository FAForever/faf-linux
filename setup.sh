...
WINE_PATH="${WINE_PATH:-/opt/wine-devel}"
...
if [[ ! -d "$STEAM_PATH" ]]; then
    echo "Could not find Steam at $STEAM_PATH. Game overlay may not work."
fi

if [[ ! -a "$WINE_PATH/bin/wine" ]]; then
    echo "Could not find wine at $WINE_PATH/bin/wine"
    exit 1
fi
